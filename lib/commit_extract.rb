...
# def clone(user, repo, update = false)
  ...
  # Remove duplicates
    ...
    f = File.join("git_travis_torrent/build_logs", "#{user}@#{repos}")
     ...
     if not builds.find { |bs| bs[:commit] == b[:now_build_commit]  }.nil? 
        b=b.merge(builds.find { |bs| bs[:commit] == b[:now_build_commit]})
     else
     ...
     diff_testing=Diff_test.new
     diff_testing.test_diff(user,repo1)
